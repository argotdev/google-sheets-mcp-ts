...
function normalizeHeader(headerRow: readonly string[]): string[] {
...
function tryCast(v: unknown): Scalar {
  ...
  if (!Number.isNaN(asInt) && String(asInt) === s) return asInt;

  const asFloat = Number.parseFloat(s);
  if (!Number.isNaN(asFloat) && Number.isFinite(asFloat)) return asFloat;
...
function recordsFromCsv(text: string, headerRow = 1): Row[] {
...
function applySelect(records: readonly Row[], select?: readonly string[] | null): Row[] {
  if (!select?.length) return records.slice();
  const cols = select.filter(Boolean);
  return records.map((r) => {
    const o: Row = {};
    for (const c of cols) o[c] = Object.prototype.hasOwnProperty.call(r, c) ? r[c] : null;
    return o;
  });
}
...
function applySort(records: readonly Row[], sort?: readonly SortKey[] | null): Row[] {
...
/** Limit/offset paging */
function page(records: readonly Row[], offset: number, limit: number): Row[] {
...
function toCsv(records: readonly Row[], includeHeader = true): string {
...
async function fetchPublishedCsv(pubId: string, gid = "0"): Promise<string> {
  ...
  async ({ pub_id, gid, filters, select, header_row = 1, format = "csv", include_header = true }, extra) => {
    try {
      ...
      const ordered = select?.length
        ? subset.map((r) => {
            const o: Row = {};
            for (const c of select) o[c] = Object.prototype.hasOwnProperty.call(r, c) ? r[c] : null;
            return o;
          })
        : subset;
